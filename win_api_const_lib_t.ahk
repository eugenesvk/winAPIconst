...
/* use winAPIconst via a loader class
  ...
  ; winAPIconst_loader.unload('embed') ; to conserve a few dozen kb memory, the DLL may be unloaded after using it (but then you'd waste time loading it on each call)
 */
...
class winAPIconst { ; Various win32 API constants from a memory-mapped file
  __new(libPath,libNm) {
    this.libPath  	:= libPath
     ...
     ,this.free   	:= DllCall.Bind(this.libNm '\dealloc_lib_struct', 'Ptr',unset)

     ,this.Loc               	:= this.Locale
     ,this.dealloc_lib_struct	:= this.free
