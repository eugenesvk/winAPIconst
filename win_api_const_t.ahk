...
; Returns value and type of a constant

; Option 1
 ; Add ‘winAPIconst_embed_t.dll’ library to your ‘lib/’ folder
 ; Launch this script to estimate the time it took to retrieve a few keys


; Option 2 [not implemented] (doesn't seem to have any benefits as the large embedded dll isn't fully loaded to memory)
 ; to use a memory-mapped database file instead of the dll with the data embedded:
 ; Add ‘winAPIconst_mmap_t.dll’ library to your ‘lib/’ folder
 ; Add the database to the same folder where this script is located or to a ‘data/’ subfolder
   ; winAPI_Const_t.rkyv_data_0
   ; winAPI_Const_t.rkyv_state
